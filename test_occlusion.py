"""
Occlusion Robustness Experiment for Reviewer #2 Q1
Run: python test_occlusion.py
"""
...
def main():
    ...
    # Look for model in saved_models directory
    ...
    # First, get baseline accuracy on clean data
    print("\n3. Computing baseline accuracy...")
    all_preds = []
    all_labels = []
    ...
    # =========================================================
    # Experiment 1: Random Joint Dropout (Occlusion)
    # =========================================================
    print("\n" + "="*60)
    print("4. Testing Random Joint Dropout (Occlusion)")
    print("="*60)
    ...
    for rate in dropout_rates:
        ...
        dropout_results[f"{int(rate*100)}%"] = acc
    ...
    # =========================================================
    print("\n" + "="*60)
    print("5. Testing Gaussian Noise (Pose Estimation Error)")
    print("="*60)
    ...
    # Summary Table
    print("\n" + "="*60)
    print("6. SUMMARY RESULTS FOR REBUTTAL")
    print("="*60)
    ...
    for rate, acc in dropout_results.items():
        if rate != "0%":
            drop = (baseline_acc - acc) * 100
            print(f"| Dropout {rate} | {acc*100:.2f} | {drop:.2f} |")
    
    for sigma, acc in noise_results.items():
        if sigma != "σ=0.0":
            drop = (baseline_acc - acc) * 100
            print(f"| Noise {sigma} | {acc*100:.2f} | {drop:.2f} |")
